...
*Setup instructions for a Gitian build of Uidd Core using a Debian VM or physical system.*
...
- [Building Uidd Core](#building-uidd-core)
...
Clone the git repositories for uidd and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/uiddproject/uidd --recursive
git clone https://github.com/uiddproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/uidd/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the uidd repository under 'Fetch and create inputs' to install sources which require
...
To build Uidd Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the uidd repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/uidd/.git/
...
For example:
```bash
URL=https://github.com/laanwj/uidd.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit uidd=${COMMIT} --url uidd=${URL} ../uidd/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit uidd=${COMMIT} --url uidd=${URL} ../uidd/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit uidd=${COMMIT} --url uidd=${URL} ../uidd/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-uidd:
./bin/gbuild --commit uidd=${COMMIT},cpp-eth-uidd=${ETHCOMMIT} --url uidd=${URL},cpp-eth-uidd=${ETHURL} ../uidd/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the uidd git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../uidd/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/uiddproject/uidd-detached-sigs.git

BTCPATH=/some/root/path/uidd
SIGPATH=/some/root/path/uidd-detached-sigs

./bin/gbuild --url uidd=${BTCPATH},signature=${SIGPATH} ../uidd/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/uidd-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/uidd-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/uidd-osx-build.assert
```
...
[uiddproject/gitian.sigs](https://github.com/uiddproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@uidd.org) and he will commit them.
