...
UiddState::UiddState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = UiddState::openDB(_path + "/uiddDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

UiddState::UiddState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute UiddState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, UiddTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createUiddAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            uidd::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> UiddState::vins() const // temp
{
...
void UiddState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* UiddState::vin(dev::Address const& _a) const
{
    return const_cast<UiddState*>(this)->vin(_a);
}

Vin* UiddState::vin(dev::Address const& _addr)
{
...
// void UiddState::commit(CommitBehaviour _commitBehaviour)
// {
...
void UiddState::kill(dev::Address _addr)
{
...
void UiddState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void UiddState::deleteAccounts(std::set<dev::Address>& addrs){
...
void UiddState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void UiddState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void UiddState::validateTransfersWithChangeLog(){
