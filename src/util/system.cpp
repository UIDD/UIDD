...
const char * const BITCOIN_CONF_FILENAME = "uidd.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Uidd
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Uidd
    // Mac: ~/Library/Application Support/Uidd
    // Unix: ~/.uidd
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Uidd";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Uidd";
#else
    // Unix
    return pathRet / ".uidd";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Uidd Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Uidd Core Developers";
    }
