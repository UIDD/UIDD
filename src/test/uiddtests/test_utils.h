...
inline void initState(){
    ...
    const std::string dirUidd = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<UiddState>(new UiddState(dev::u256(0), UiddState::openDB(dirUidd, hashDB, dev::WithExisting::Trust), dirUidd + "/uiddDB", dev::eth::BaseState::Empty));
...
inline dev::Address createUiddAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline UiddTransaction createUiddTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    UiddTransaction txEth;
    if(recipient == dev::Address()){
        txEth = UiddTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = UiddTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<UiddTransaction> txs){
