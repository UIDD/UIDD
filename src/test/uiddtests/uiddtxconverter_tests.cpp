...
void checkResult(bool isCreation, std::vector<UiddTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    UiddTxConverter converter(transaction, NULL);
    ExtractUiddTX uiddTx;
    BOOST_CHECK(converter.extractionUiddTransactions(uiddTx));
    std::vector<UiddTransaction> result = uiddTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(uiddtxconverter_tests, TestingSetup)
