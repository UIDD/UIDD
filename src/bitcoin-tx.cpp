...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " uidd-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  uidd-tx [options] <hex-tx> [commands]  Update hex-encoded uidd transaction\n" +
            "or:     uidd-tx [options] -create [commands]   Create hex-encoded uidd transaction\n" +
            "\n";
