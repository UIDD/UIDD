...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setUiddSchedule(uiddDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = uiddDGP.getBlockSize(nHeight);
    minGasPrice = uiddDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = uiddDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    UiddTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractUiddTX resultConverter;
    if(!convert.extractionUiddTransactions(resultConverter)){
    ...
    for(UiddTransaction uiddTransaction : uiddTransactions){
        txGas += uiddTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + uiddTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(uiddTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, uiddTransactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "uiddstake";
...
void StakeUidds(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
