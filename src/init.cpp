 ...
 */
static const char* BITCOIN_PID_FILENAME = "uiddd.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/uiddproject/uidd>";
    const std::string URL_WEBSITE = "<https://uidd.org>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateUidd");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path uiddStateDir = GetDataDir() / "stateUidd";
                bool fStatus = fs::exists(uiddStateDir);
                const std::string dirUidd(uiddStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsUiddstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<UiddState>(new UiddState(dev::u256(0), UiddState::openDB(dirUidd, hashDB, dev::WithExisting::Trust), dirUidd, existsUiddstate));
                dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::uiddMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(uiddStateDir.string()));
                if (fReset) {
