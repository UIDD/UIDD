...
#include <qt/uiddversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    uiddVersionChecker = new UiddVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Uidd address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and uidd: URIs)"));
    ...
    historyAction = new QAction(platformStyle->MultiStatesIcon(":/icons/history"), tr("&Transactions"), this);//UIDD_LINE
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Uidd addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Uidd addresses"));
    ...
    openAction->setStatusTip(tr("Open a uidd: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Uidd command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && uiddVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(UIDD_RELEASES, UIDD_RELEASES);
            QString message(tr("New version of Uidd wallet is available on the Uidd source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Uidd network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Uidd"); // default title
